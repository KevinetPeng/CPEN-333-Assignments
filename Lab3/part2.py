...
if __name__ == "__main__":
    ...
    # Create list of type Process to join all processes later
    processes = []

    for col in range(SIZE):  
        # create and start process for each col
        p = multiprocessing.Process(target=checkColumn, args=(testcase, col))
        p.start()

        # Append to list to join later
        processes.append(p)

    for row in range(SIZE):  
        # create and start process for each row
    ...
    for subgrid in range(SIZE):   
        # create and start process for each subgrid
