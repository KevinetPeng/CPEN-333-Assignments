...
def producer() -> None:
    ...
    for _ in range(SIZE * 2): #we just produce twice the buffer size for testing
        ...
        # acquire empty semaphore to ensure there's enough space in the buffer before proceeding
        empty.acquire()

        # acquire lock before inserting into buffer
        mutex.acquire()
        try:
            buffer.insert(item)
        finally:
        ...
        # release full semaphore when finished because item count incremented by 1
        full.release()
...
def consumer() -> None:
    ...
    for _ in range(SIZE * 2): #we just consume twice the buffer size for testing
        ...
        # acquire full semaphore to ensure at least 1 element is in the buffer before proceeding
        full.acquire()
    
        # acquire lock before removing item from buffer
        mutex.acquire()
        try:
            item = buffer.remove()
        finally:
        ...
        # release empty semaphore because number of items buffer decreased by 1
        empty.release()
