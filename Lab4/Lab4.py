...
def mergingWorker() -> None:
    ...
    # while both arrays are not empty, add lowest current value from either array using current indices
    while leftIndex < len(sortedFirstHalf) and rightIndex < len(sortedSecondHalf):
      if sortedFirstHalf[leftIndex] < sortedSecondHalf[rightIndex]:
        SortedFullList.append(sortedFirstHalf[leftIndex])
        leftIndex += 1
      else:
        SortedFullList.append(sortedSecondHalf[rightIndex])
        rightIndex += 1
    
    # whatever is left in either leftArray or rightArray starting from the remaining indices will be sorted and greater than our sortedMerge array
    SortedFullList.extend(sortedFirstHalf[leftIndex:])
    SortedFullList.extend(sortedSecondHalf[rightIndex:])
